...
<div class="form-card">
    <div class="form-header">
        <h3 class="form-title">Add PG Room</h3>
        <a href="{{ route('superadmin.pg-rooms.index') }}" class="btn-secondary">← Back</a>
    </div>
    @if ($errors->any()) <div class="alert alert-danger"><ul>@foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div> @endif

    <form action="{{ route('superadmin.pg-rooms.store') }}" method="POST">
        @csrf
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="room_no">Room Number *</label>
                    <input type="text" name="room_no" id="room_no" class="form-control" value="{{ old('room_no') }}" required placeholder="e.g. 101">
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <select name="room_type" id="room_type" class="form-control" required>
                        ...
                        <option value="dorm" {{ old('room_type') == 'dorm' ? 'selected' : '' }}>Dormitory</option>
                    </select>
                </div>
            </div>
        </div>
